...
- **Plan** — the scene map: your logged reads as pins on an OpenStreetMap view (Leaflet, vendored locally), tap anywhere to scout a spot — every pin gets that location's sun windows per look (same math as Now, computed for the pin's coordinates and weather) and a gold ray showing where the sun sits at its golden hour. Save pin + look + window as a shot; the shot list orders itself by light and exports as a day-plan PNG. Scene memory (the last 12 reads) lives here too.
