...
async function main() {
    // Scrim: boot with no scene when ?url= is absent — the page's import
    // button feeds selectFile() instead, so nothing 404s on an empty start.
    const params = new URLSearchParams(location.search);
    const urlParam = params.get("url");
    let carousel = !!urlParam;
    try {
    ...
    const frame = (now) => {
        ...
        if (vertexCount > 0) {
            ...
            gl.clear(gl.COLOR_BUFFER_BIT);
            // only spin when something is actually loading; an empty boot
            // waits quietly for an import
            document.getElementById("spinner").style.display =
                splatData.length > 0 ? "" : "none";
            start = Date.now() + 2000;
        }
        // Scrim: hand the page this frame's matrices so it can project the
        // sun path into the same scene
        if (window.__scanFrame)
            window.__scanFrame(actualViewMatrix, projectionMatrix, avgFps, vertexCount);
        const progress = splatData.length
    ...
    // Scrim: the stock default view sits inside most room scans on a phone's
    // narrow FOV — start pulled back so the space reads immediately.
    const startView = () => {
        let inv = invert4(defaultViewMatrix);
        inv = translate4(inv, 0, 0, -2.5);
        return invert4(inv);
    };

    const selectFile = (file) => {
        carousel = false;
        viewMatrix = startView();
    ...
    // Scrim: the page's import button and reset control drive the viewer
    window.__scanSelectFile = selectFile;
    window.__scanResetView = () => {
        viewMatrix = startView();
        carousel = false;
    };
    ...
    if (!reader) return; // empty boot: everything from here is URL streaming
